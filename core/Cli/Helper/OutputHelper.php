...
use Footup\Cli\Exception\Exception;
use Footup\Cli\Input\Argument;
use Footup\Cli\Input\Command;
use Footup\Cli\Input\Groupable;
...
class OutputHelper
{
     ...
     */
    public function showUsage(string $usage, $command): self
    {
        // old manner
        // $usage = str_replace('$0', $_SERVER['argv'][1] ?? '[cmd]', $usage);
        $usage = str_replace('$0', $command->name() , $usage);

        if (stripos($usage, ' ## ') === false) {
