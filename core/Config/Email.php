...
/**
 * This file was part of the CodeIgniter 4 framework.
 *
...
use App\Config\Email as EmailConfig;
use ErrorException;
...
class Email
{
	 ...
	 *
	 * @param array|\App\Config\Email $config
	 *
	 * @return Email
	 */
	...
	protected function spoolEmail()
	{
		...
		{
			$success = false;
			throw new ErrorException('Email: ' . $method . ' throwed ' . $e->getMessage());
		}
	...
	protected function sendData($data)
	{
		...
		for ($written = $timestamp = $result = 0, $length = static::strlen($data); $written < $length; $written += $result)
		{
	...
	{
		$mime = Mime::getMime(strtolower($ext)) ?? "";
